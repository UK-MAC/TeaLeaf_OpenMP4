!Crown Copyright 2014 AWE.
!
! This file is part of TeaLeaf.
!
! TeaLeaf is free software: you can redistribute it and/or modify it under 
! the terms of the GNU General Public License as published by the 
! Free Software Foundation, either version 3 of the License, or (at your option) 
! any later version.
!
! TeaLeaf is distributed in the hope that it will be useful, but 
! WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or 
! FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License for more 
! details.
!
! You should have received a copy of the GNU General Public License along with 
! TeaLeaf. If not, see http://www.gnu.org/licenses/.

!>  @brief Driver for the heat conduction kernel
!>  @author David Beckingsale, Wayne Gaudin, Matthew Martineau
!>  @details Invokes the user specified kernel for the heat conduction

MODULE tea_leaf_module

    USE report_module
    USE data_module
    USE tea_leaf_kernel_module
    USE tea_leaf_kernel_cg_module
    USE tea_leaf_kernel_ppcg_module
    USE tea_leaf_kernel_cheby_module
    USE update_halo_module

    IMPLICIT NONE

CONTAINS

    SUBROUTINE tea_leaf()

        IMPLICIT NONE

        REAL(KIND=8) :: timer
        REAL(KIND=8) :: cg_time, ch_time, total_solve_time, ch_per_it, cg_per_it, iteration_time

        INTEGER :: c, n
        INTEGER :: cg_calc_steps
        INTEGER :: est_itc, cheby_calc_steps

        cg_time = 0.0_8
        ch_time = 0.0_8
        total_solve_time = 0.0_8

        IF(coefficient .NE. RECIP_CONDUCTIVITY .AND. coefficient .NE. conductivity) THEN
            CALL report_error('tea_leaf', 'unknown coefficient option')
        ENDIF

        cheby_calc_steps = 0
        cg_calc_steps = 0

        total_solve_time = timer()

        c=1

        CALL solve_chunk(c, n, cg_calc_steps, est_itc, &
            cheby_calc_steps, cg_time, ch_time, iteration_time, &
            chunks(c)%task, &
            chunks(c)%chunk_neighbours, &
            chunks(c)%field%x_min, &
            chunks(c)%field%y_min, &
            chunks(c)%field%x_max, &
            chunks(c)%field%y_max, &
            chunks(c)%field%density, &
            chunks(c)%field%energy0, &
            chunks(c)%field%energy1, &
            chunks(c)%field%u, &
            chunks(c)%field%u0, &
            chunks(c)%field%vector_p, &
            chunks(c)%field%vector_r, &
            chunks(c)%field%vector_Mi, &
            chunks(c)%field%vector_w, &
            chunks(c)%field%vector_z, &
            chunks(c)%field%vector_Kx, &
            chunks(c)%field%vector_Ky, &
            chunks(c)%field%vector_sd, &
            parallel%dev_id)

        IF (profiler_on .AND. parallel%boss) THEN
            total_solve_time = (timer() - total_solve_time)
            WRITE(0, "(a16, a7, a16)") "Time", "Steps", "Per it"
            WRITE(0, "(f16.10, i7, f16.10, f7.2)") total_solve_time, n, total_solve_time/n
            WRITE(g_out, "(a16, a7, a16)") "Time", "Steps", "Per it"
            WRITE(g_out, "(f16.10, i7, f16.10, f7.2)") total_solve_time, n, total_solve_time/n
        ENDIF

        IF (profiler_on .AND. tl_use_chebyshev) THEN
            CALL tea_sum(ch_time)
            CALL tea_sum(cg_time)
            IF (parallel%boss) THEN
                cg_per_it = MERGE((cg_time/cg_calc_steps)/parallel%max_task, &
                    0.0_8, cg_calc_steps .GT. 0)
                ch_per_it = MERGE((ch_time/cheby_calc_steps)/parallel%max_task, &
                    0.0_8, cheby_calc_steps .GT. 0)

                WRITE(0, "(a3, a16, a7, a16, a7)") "", "Time", "Steps", "Per it", "Ratio"
                WRITE(0, "(a3, f16.10, i7, f16.10, f7.2)") &
                    "CG", cg_time + 0.0_8, cg_calc_steps, cg_per_it, 1.0_8
                WRITE(0, "(a3, f16.10, i7, f16.10, f7.2)") "CH", ch_time+0.0_8,cheby_calc_steps, &
                    ch_per_it, MERGE(ch_per_it/cg_per_it, 0.0_8, cheby_calc_steps .GT. 0)
                WRITE(0, "('Chebyshev actually took ', i6, ' (' i6, ' off guess)')") &
                    cheby_calc_steps, cheby_calc_steps-est_itc

                WRITE(g_out, "(a3, a16, a7, a16, a7)") "", "Time", "Steps", "Per it", "Ratio"
                WRITE(g_out, "(a3, f16.10, i7, f16.10, f7.2)") &
                    "CG", cg_time + 0.0_8, cg_calc_steps, cg_per_it, 1.0_8
                WRITE(g_out, "(a3, f16.10, i7, f16.10, f7.2)") "CH", ch_time+0.0_8, &
                    cheby_calc_steps, ch_per_it, MERGE(ch_per_it/cg_per_it, 0.0_8, &
                    cheby_calc_steps .GT. 0)
                WRITE(g_out, "('Chebyshev actually took ', i6, ' (' i6, ' off guess)')") &
                    cheby_calc_steps, cheby_calc_steps-est_itc
            ENDIF
        ENDIF

    END SUBROUTINE tea_leaf

    SUBROUTINE solve_chunk(c, n, cg_calc_steps, est_itc, cheby_calc_steps, &
            cg_time, ch_time, iteration_time, task, chunk_neighbours, x_min, &
            y_min, x_max, y_max, density, energy0, energy1, u, u0, p, r, Mi, w, z, Kx, &
            Ky, sd, dev_id)

        IMPLICIT NONE

        !$ INTEGER :: OMP_GET_THREAD_NUM
        INTEGER :: c, n, cg_calc_steps
        INTEGER :: est_itc, cheby_calc_steps, task, dev_id
        REAL(KIND=8) :: cg_time, ch_time, iteration_time, timer

        INTEGER :: fields(NUM_FIELDS)
        INTEGER :: chunk_neighbours(4)
        INTEGER :: x_min, y_min, x_max, y_max
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: density
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: energy0,energy1
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: u,u0
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: p
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: r
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: Mi
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: w
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: z
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: Kx
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: Ky
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: sd

        LOGICAL :: ch_switch_check
        INTEGER :: max_cheby_iters, switch_step, info
        REAL(KIND=8) :: ry, rx, exact_error, error
        REAL(KIND=8) :: halo_time,solve_time,init_time,reset_time
        REAL(KIND=8) :: rro, pw, rrn, alpha, beta
        REAL(KIND=8) :: eigmin, eigmax, theta, cn
        REAL(KIND=8), DIMENSION(max_iters) :: cg_alphas, cg_betas
        REAL(KIND=8), DIMENSION(max_iters) :: ch_alphas, ch_betas

        error = 1e10
        time = 0.0_8
        init_time = 0.0_8
        halo_time = 0.0_8
        solve_time = 0.0_8

        IF(task.EQ.parallel%task) THEN
            !$OMP TARGET UPDATE DEVICE(dev_id) TO(max_iters, g_out, parallel, profiler_on)&
            !$OMP TO(dx, dy, chunks, use_ext_kernels, tl_use_chebyshev, tl_use_cg) &
            !$OMP TO(tl_use_ppcg, tl_use_jacobi, use_fortran_kernels, eps, coefficient) &
            !$OMP TO(tl_ch_cg_errswitch, tl_ch_cg_epslim, tl_ch_cg_presteps) &
            !$OMP TO(tl_check_result, tl_ppcg_inner_steps, tl_preconditioner_on) &
            !$OMP TO(complete, profiler, dt)

            !$OMP TARGET DATA DEVICE(dev_id) &
            !$OMP MAP(to: chunk_neighbours, fields, n, r, sd, Kx, Ky)&
            !$OMP MAP(to: Mi, z, w, p, ch_alphas, ch_betas, cg_alphas, cg_betas)&
            !$OMP MAP(tofrom: density, energy1, energy0, u, u0)

            IF (profiler_on) halo_time=timer()
            fields=0
            fields(FIELD_ENERGY1) = 1
            fields(FIELD_DENSITY) = 1

            CALL update_halo(fields, 2, c, x_min, x_max, y_min, y_max, chunk_neighbours, &
                density, u, energy0, energy1, p, sd,1,dev_id)

            IF (profiler_on) profiler%halo_exchange = profiler%halo_exchange + &
                (timer() - halo_time)
            IF (profiler_on) init_time=timer()

            rx = dt/(dx**2)
            ry = dt/(dy**2)

            IF(tl_use_cg .OR. tl_use_chebyshev .OR. tl_use_ppcg) THEN
                ! All 3 of these solvers use the CG kernels
                IF(use_ext_kernels) THEN
                    CALL ext_cg_solver_init(c, density, energy1, u, p, r, Mi, &
                        w, z, Kx, Ky, coefficient, tl_preconditioner_on, dt, &
                        rx, ry, rro,dev_id)
                ELSEIF(use_fortran_kernels) THEN
                    CALL tea_leaf_kernel_init_cg_fortran(x_min, x_max, y_min, y_max, &
                        density, energy1, u, p, r, Mi, w, z, Kx, Ky, rx, ry, rro, &
                        coefficient, tl_preconditioner_on,dev_id)
                ENDIF

                ! need to update p when using CG due to matrix/vector multiplication
                fields=0
                fields(FIELD_U) = 1
                fields(FIELD_P) = 1
                IF (profiler_on) halo_time=timer()
                CALL update_halo(fields,1,c,x_min,x_max,y_min,y_max,chunk_neighbours,&
                    density,u,energy0,energy1,p,sd,1,dev_id)

                IF (profiler_on) profiler%halo_exchange = profiler%halo_exchange + &
                    (timer() - halo_time)
                init_time=init_time+(timer()-halo_time)

                ! and globally sum rro
                CALL tea_allsum(rro)
            ELSEIF(tl_use_jacobi) THEN
                IF(use_ext_kernels) THEN
                    CALL ext_jacobi_kernel_init(c, density, energy1, u0, u, r, Kx, &
                        Ky, coefficient, dt, rx, ry,dev_id)
                ELSEIF (use_fortran_kernels) THEN
                    CALL tea_leaf_kernel_init(x_min, x_max, y_min, y_max, density, &
                        energy1, u0, u, r, Kx, Ky, coefficient,dev_id)
                ENDIF

                fields=0
                fields(FIELD_U) = 1
            ENDIF

            ! need the original value of u
            IF(use_ext_kernels) THEN
                CALL ext_solver_copy_u(c, u0, u, dev_id)
            ELSEIF(use_fortran_kernels) THEN
                CALL tea_leaf_kernel_cheby_copy_u(x_min, x_max, y_min, y_max, u0, u, dev_id)
            ENDIF

            IF (profiler_on) profiler%tea_init = profiler%tea_init + (timer() - init_time)
            IF (profiler_on) solve_time = timer()

            DO n=1,max_iters
                iteration_time = timer()

                IF (tl_ch_cg_errswitch) THEN
                    ! either the error has got below tolerance, or it's already going -
                    !  minimum 20 steps to converge eigenvalues
                    ch_switch_check = (cheby_calc_steps .GT. 0) .OR. &
                        (error .LE. tl_ch_cg_epslim) .AND. (n .GE. 20)
                ELSE
                    ! enough steps have passed and error < 1, otherwise it's 
                    ! nowhere near converging on eigenvalues
                    ch_switch_check = (n .GE. tl_ch_cg_presteps) .AND. (error .le. 1.0_8)
                ENDIF

                IF ((tl_use_chebyshev .OR. tl_use_ppcg) .AND. ch_switch_check) THEN
                    ! on the first chebyshev steps, find the eigenvalues, coefficients,
                    ! and expected number of iterations
                    IF (cheby_calc_steps .EQ. 0) THEN
                        ! maximum number of iterations in chebyshev solver
                        max_cheby_iters = max_iters - n + 2
                        rro = error

                        ! calculate eigenvalues
                        CALL tea_calc_eigenvalues(cg_alphas, cg_betas, eigmin, eigmax, &
                            max_iters, n-1, info)

                        IF (info .NE. 0) THEN
                            CALL report_error('tea_leaf', 'Error in calculating eigenvalues')
                            WRITE(0,*) 'Error in calculating eigenvalues'
                            STOP 0
                        ENDIF

                        eigmin = eigmin*0.95
                        eigmax = eigmax*1.05

                        IF (tl_use_chebyshev) THEN
                            ! calculate chebyshev coefficients
                            CALL tea_calc_ch_coefs(ch_alphas, ch_betas, eigmin, eigmax, &
                                theta, max_cheby_iters)

                            ! don't need to update p any more
                            fields = 0
                            fields(FIELD_U) = 1
                        ELSE IF (tl_use_ppcg) THEN
                            ! currently also calculate chebyshev coefficients
                            ! TODO least squares
                            CALL tea_calc_ch_coefs(ch_alphas, ch_betas, eigmin, eigmax, &
                                theta, tl_ppcg_inner_steps)

                        ENDIF

                        cn = eigmax/eigmin

                        IF (parallel%boss) THEN
                            WRITE(g_out,'(a,i3,a,e15.7)') "Switching after ",n," steps, error ",rro
                            WRITE(g_out,'(4a11)')"eigmin", "eigmax", "cn", "error"
                            WRITE(g_out,'(2f11.5,2e11.4)')eigmin, eigmax, cn, error
                            WRITE(0,'(a,i3,a,e15.7)') "Switching after ",n," steps, error ",rro
                            WRITE(0,'(4a11)')"eigmin", "eigmax", "cn", "error"
                            WRITE(0,'(2f11.5,2e11.4)')eigmin, eigmax, cn, error
                        ENDIF
                    ENDIF

                    IF (tl_use_chebyshev) THEN
                        IF (cheby_calc_steps .EQ. 0) THEN
                            CALL tea_leaf_cheby_first_step(c, ch_alphas, ch_betas, fields, &
                                chunk_neighbours, error, rx, ry, theta, cn, max_cheby_iters, &
                                est_itc, x_min,  y_min, x_max, y_max, r, sd, density, &
                                energy0, energy1, u, u0, Kx, Ky, p, Mi, w, z, dev_id)

                            cheby_calc_steps = 1
                            switch_step = n
                        ELSE
                            IF(use_ext_kernels) THEN
                                CALL ext_cheby_solver_iterate(c, u, u0, p, r, Mi, &
                                    w, z, Kx, Ky, ch_alphas, &
                                    ch_betas, max_cheby_iters, max_cheby_iters, dev_id)
                            ELSE IF(use_fortran_kernels) THEN
                                CALL tea_leaf_kernel_cheby_iterate(x_min, x_max, y_min, y_max, &
                                    u, u0, p, r, Mi, w, z, Kx,&
                                    Ky, ch_alphas, ch_betas, max_cheby_iters, rx, ry, &
                                    cheby_calc_steps, tl_preconditioner_on, dev_id)
                            ENDIF

                            ! after estimated number of iterations has passed, calc resid.
                            ! Leaving 10 iterations between each global reduction won't affect
                            ! total time spent much if at all (number of steps spent in
                            ! chebyshev is typically O(300+)) but will greatly reduce global
                            ! synchronisations needed
                            IF ((n .GE. est_itc) .AND. (MOD(n, 10) .eq. 0)) THEN
                                IF(use_ext_kernels) THEN
                                    CALL ext_calculate_2norm(c, r, error, dev_id)
                                ELSE IF(use_fortran_kernels) THEN
                                    CALL tea_leaf_calc_2norm_kernel(x_min, x_max, y_min, y_max, &
                                        r, error, dev_id)
                                ENDIF

                                CALL tea_allsum(error)
                            ENDIF
                        ENDIF
                    ELSE IF (tl_use_ppcg) THEN
                        IF (cheby_calc_steps .EQ. 0) THEN
                            cheby_calc_steps = 1

                            IF(use_ext_kernels) THEN
                                CALL ext_calculate_residual(c, u, u0, r, Kx, Ky, dev_id)                
                            ELSE IF(use_fortran_kernels) THEN
                                CALL tea_leaf_calc_residual(x_min, x_max, y_min, y_max,  u, &
                                    u0, r, Kx, Ky, rx, ry, dev_id)
                            ENDIF

                            IF (profiler_on) halo_time = timer()
                            ! update p
                            CALL update_halo(fields,1,c,x_min,x_max,y_min,y_max,&
                                chunk_neighbours,density,u,energy0,energy1,p,sd,1, dev_id)
                            IF (profiler_on) profiler%halo_exchange = profiler%halo_exchange + &
                                (timer() - halo_time)
                            IF (profiler_on) solve_time = solve_time + (timer()-halo_time)

                            CALL tea_allsum(rro)
                        ENDIF

                        IF(use_ext_kernels) THEN
                            CALL ext_cg_calc_w(c, p, w, Kx, Ky, pw, dev_id)
                        ELSE IF(use_fortran_kernels) THEN
                            CALL tea_leaf_kernel_solve_cg_fortran_calc_w(x_min, x_max, y_min, &
                                y_max, p, w, Kx, Ky, rx, ry, pw, dev_id)
                        ENDIF

                        CALL tea_allsum(pw)
                        alpha = rro/pw

                        IF(use_ext_kernels) THEN
                            CALL ext_cg_calc_ur(c, u, p, r, Mi, w, z, alpha, rrn, dev_id) 
                        ELSE IF(use_fortran_kernels) THEN
                            CALL tea_leaf_kernel_solve_cg_fortran_calc_ur(x_min, x_max, &
                                y_min, y_max, u, p, r, Mi, w, z, alpha, rrn, &
                                tl_preconditioner_on, dev_id)
                        ENDIF

                        ! not using rrn, so don't do a tea_allsum

                        CALL tea_leaf_run_ppcg_inner_steps(ch_alphas, ch_betas, theta, &
                            rx, ry, tl_ppcg_inner_steps, c, chunk_neighbours, x_min, y_min, &
                            x_max, y_max, r, sd, density, energy0, energy1, u, &
                            Kx, Ky, p, Mi, w, z, dev_id)

                        IF(use_ext_kernels) THEN
                            CALL ext_calculate_2norm(c, r, rrn, dev_id)
                        ELSE IF(use_fortran_kernels) THEN
                            CALL tea_leaf_calc_2norm_kernel(x_min, x_max, y_min, y_max, &
                                r, rrn, dev_id)
                        ENDIF

                        CALL tea_allsum(rrn)

                        beta = rrn/rro

                        IF(use_ext_kernels) THEN
                            CALL ext_cg_calc_p(c, p, r, z, beta, dev_id)
                        ELSE IF(use_fortran_kernels) THEN
                            CALL tea_leaf_kernel_solve_cg_fortran_calc_p(x_min, x_max, y_min, &
                                y_max, p, r, z, beta, tl_preconditioner_on, dev_id)
                        ENDIF

                        error = rrn
                        rro = rrn
                    ENDIF

                    cheby_calc_steps = cheby_calc_steps + 1
                ELSEIF(tl_use_cg .OR. tl_use_chebyshev .OR. tl_use_ppcg) THEN
                    fields(FIELD_P) = 1
                    cg_calc_steps = cg_calc_steps + 1

                    IF(use_ext_kernels) THEN
                        CALL ext_cg_calc_w(c, p, w, Kx, Ky, pw, dev_id)
                    ELSE IF(use_fortran_kernels) THEN
                        CALL tea_leaf_kernel_solve_cg_fortran_calc_w(x_min, x_max, y_min, &
                            y_max, p, w, Kx, Ky, rx, ry, pw, dev_id)
                    ENDIF

                    CALL tea_allsum(pw)
                    alpha = rro/pw
                    IF(tl_use_chebyshev .OR. tl_use_ppcg) THEN
                        cg_alphas(n) = alpha
                    ENDIF

                    IF(use_ext_kernels) THEN
                        CALL ext_cg_calc_ur(c, u, p, r, Mi, w, z, alpha, rrn, dev_id) 
                    ELSE IF(use_fortran_kernels) THEN
                        CALL tea_leaf_kernel_solve_cg_fortran_calc_ur(x_min, x_max, y_min, &
                            y_max, u, p, r, Mi, w, z, alpha, rrn, tl_preconditioner_on, dev_id)
                    ENDIF

                    CALL tea_allsum(rrn)
                    beta = rrn/rro

                    IF(tl_use_chebyshev .OR. tl_use_ppcg) THEN
                        cg_betas(n) = beta
                    ENDIF

                    IF(use_ext_kernels) THEN
                        CALL ext_cg_calc_p(c, p, r, z, beta, dev_id)
                    ELSE IF(use_fortran_kernels) THEN
                        CALL tea_leaf_kernel_solve_cg_fortran_calc_p(x_min, x_max, y_min, &
                            y_max, p, r, z, beta, tl_preconditioner_on, dev_id)
                    ENDIF

                    error = rrn
                    rro = rrn
                ELSEIF(tl_use_jacobi) THEN
                    IF(use_ext_kernels) THEN
                        CALL ext_jacobi_kernel_solve(c, Kx, Ky, u0, u, r, error, dev_id)
                    ELSEIF(use_fortran_kernels) THEN
                        CALL tea_leaf_kernel_solve(x_min, x_max, y_min, y_max, rx, ry, &
                            Kx, Ky, error, u0, u, r, dev_id)
                    ENDIF

                    ! error for jacobi is calculated recursively and is not very accurate,
                    ! so do this every so often to see whether it has actually converged
                    IF (mod(n, 50) .eq. 0) THEN
                        CALL update_halo(fields,1,c,x_min,x_max,y_min,y_max, &
                            chunk_neighbours,density,u,energy0,energy1,p,sd,1, dev_id)

                        IF(use_ext_kernels) THEN
                            CALL ext_calculate_residual(c, u, u0, r, Kx, Ky, dev_id)
                            CALL ext_calculate_2norm(c, r, error, dev_id)
                        ELSEIF(use_fortran_kernels) THEN
                            CALL tea_leaf_calc_residual(x_min, x_max, y_min, y_max, u, u0,&
                                r, Kx, Ky, rx, ry, dev_id)
                            call tea_leaf_calc_2norm_kernel(x_min, x_max, y_min, y_max, &
                                r, error, dev_id)
                        ENDIF
                    ENDIF

                    CALL tea_allsum(error)
                ENDIF

                ! updates u and possibly p
                IF (profiler_on) halo_time = timer()
                CALL update_halo(fields, 1, c, x_min, x_max, y_min, y_max, &
                    chunk_neighbours, density, u, energy0, energy1, p, sd,1, dev_id)
                IF (profiler_on) profiler%halo_exchange = profiler%halo_exchange + &
                    (timer() - halo_time)
                IF (profiler_on) solve_time = solve_time + (timer()-halo_time)

                IF (profiler_on) THEN
                    IF (tl_use_chebyshev .AND. ch_switch_check) THEN
                        ch_time=ch_time+(timer()-iteration_time)
                    ELSE
                        cg_time=cg_time+(timer()-iteration_time)
                    ENDIF
                ENDIF

                IF (abs(error) .LT. eps) EXIT

            ENDDO

            IF (tl_check_result) THEN
                IF(use_ext_kernels) THEN
                    CALL ext_calculate_residual(c, u, u0, r, Kx, Ky, dev_id)                    
                    CALL ext_calculate_2norm(c,r, exact_error, dev_id)
                ELSEIF(use_fortran_kernels) THEN
                    CALL tea_leaf_calc_residual(x_min, x_max, y_min, y_max, u, u0,&
                        r, Kx, Ky, rx, ry, dev_id)
                    CALL tea_leaf_calc_2norm_kernel(x_min, x_max, y_min, y_max, &
                        r, exact_error, dev_id)
                ENDIF

                CALL tea_allsum(exact_error)
            ENDIF

            IF (profiler_on) profiler%tea_solve = profiler%tea_solve + (timer() - solve_time)

            IF (parallel%boss) THEN
                !$      IF(OMP_GET_THREAD_NUM().EQ.0) THEN
                WRITE(g_out,"('Conduction error ',e14.7)") error
                WRITE(0,"('Conduction error ',e14.7)") error

                IF (tl_check_result) THEN
                    WRITE(0,"('EXACT error calculated as', e14.7)") exact_error
                    WRITE(g_out,"('EXACT error calculated as', e14.7)") exact_error
                ENDIF

                WRITE(g_out,"('Iteration count ',i8)") n-1
                WRITE(0,"('Iteration count ', i8)") n-1
                !$      ENDIF
            ENDIF

            ! RESET
            reset_time=timer()
            IF(use_ext_kernels) THEN
                CALL ext_solver_finalise(c, energy1, density, u, dev_id)
            ELSEIF(use_fortran_kernels) THEN
                CALL tea_leaf_kernel_finalise(x_min, x_max, y_min, y_max, &
                    energy1, density, u, dev_id)
            ENDIF
            IF (profiler_on) profiler%tea_reset = profiler%tea_reset + (timer() - halo_time) 
            halo_time=timer()
            fields=0
            fields(FIELD_ENERGY1) = 1
            CALL update_halo(fields,1,c,x_min,x_max,y_min,y_max, &
                chunk_neighbours,density,u,energy0,energy1,p,sd,1, dev_id)
            IF (profiler_on) profiler%halo_exchange = profiler%halo_exchange + &
                (timer() - halo_time)

            !$OMP END TARGET DATA

        ENDIF

    END SUBROUTINE

    SUBROUTINE tea_leaf_run_ppcg_inner_steps(ch_alphas, ch_betas, theta, &
            rx, ry, tl_ppcg_inner_steps, c, chunk_neighbours, x_min, y_min, x_max, &
            y_max, r, sd, density, energy0, energy1, u, &
            Kx, Ky, p, Mi, w, z, dev_id)

        INTEGER :: fields(NUM_FIELDS), dev_id
        INTEGER :: c, tl_ppcg_inner_steps, ppcg_cur_step
        REAL(KIND=8) :: rx, ry, theta
        REAL(KIND=8), DIMENSION(max_iters) :: ch_alphas, ch_betas
        INTEGER :: x_min, y_min, x_max, y_max
        INTEGER :: chunk_neighbours(4)
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: density
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: energy0,energy1
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: r
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: sd
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: u
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: Kx
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: Ky
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: p
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: Mi
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: w
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: z

        IF(use_ext_kernels) THEN
            CALL ext_ppcg_init_sd(c,theta,r,sd,mi, dev_id)
        ELSE IF(use_fortran_kernels) THEN
            CALL tea_leaf_kernel_ppcg_init_sd(x_min, x_max, y_min, y_max, r, &
                sd, theta, dev_id)
        ENDIF

        fields = 0
        fields(FIELD_SD) = 1

        ! inner steps
        DO ppcg_cur_step=1,tl_ppcg_inner_steps
            CALL update_halo(fields,1,c,x_min,x_max,y_min,y_max,&
                chunk_neighbours,density,u,energy0,energy1,p,sd,1, dev_id)

            IF(use_ext_kernels) THEN
                CALL ext_ppcg_inner(c, u, r, kx, ky, sd, mi, ch_alphas, ch_betas, &
                    ppcg_cur_step,tl_ppcg_inner_steps, dev_id)
            ELSE IF(use_fortran_kernels) THEN
                call tea_leaf_kernel_ppcg_inner(x_min, x_max, y_min, y_max,&
                    ppcg_cur_step, ch_alphas, ch_betas, rx, ry, u, r, Kx,&
                    Ky, sd, dev_id)
            ENDIF
        ENDDO

        fields = 0
        fields(FIELD_P) = 1

    END SUBROUTINE tea_leaF_run_ppcg_inner_steps

    SUBROUTINE tea_leaf_cheby_first_step(c, ch_alphas, ch_betas, fields, chunk_neighbours,&
            error, rx, ry, theta, cn, max_cheby_iters, est_itc, x_min, y_min, x_max, y_max,&
            density, energy0, energy1, r, sd, u, u0, Kx, Ky, p, Mi, w, z, dev_id)

        IMPLICIT NONE

        integer :: c, est_itc, max_cheby_iters, dev_id
        integer, dimension(:) :: fields
        REAL(KIND=8) :: it_alpha, cn, gamm, bb, error, rx, ry, theta
        REAL(KIND=8), DIMENSION(:) :: ch_alphas, ch_betas
        INTEGER :: x_min, y_min, x_max, y_max
        INTEGER :: chunk_neighbours(4)
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: density
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: energy0,energy1
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: r
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: sd
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: u, u0
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: Kx
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: Ky
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: p
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: Mi
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: w
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: z

        ! calculate 2 norm of u0
        IF(use_ext_kernels) THEN
            CALL ext_calculate_2norm(c, u0, bb, dev_id)
        ELSE IF(use_fortran_kernels) THEN
            CALL tea_leaf_calc_2norm_kernel(x_min, x_max, y_min, y_max, u0, bb, dev_id)
        ENDIF

        CALL tea_allsum(bb)

        ! initialise 'p' array
        IF(use_ext_kernels) THEN
            CALL ext_cheby_solver_init(c, u, u0, p, r, Mi, w, z,&
                Kx, Ky, theta, tl_preconditioner_on, dev_id)
        ELSE IF(use_fortran_kernels) THEN
            CALL tea_leaf_kernel_cheby_init(x_min, x_max, y_min, y_max, u, u0, p, &
                r, Mi, w, z, Kx, Ky, ch_alphas, ch_betas, max_cheby_iters, rx, ry, &
                theta, error, tl_preconditioner_on, dev_id)
        ENDIF

        CALL update_halo(fields,1,c,x_min,x_max,y_min,y_max,&
            chunk_neighbours,density,u,energy0,energy1,p,sd,1, dev_id)

        IF(use_ext_kernels) THEN
            CALL ext_cheby_solver_iterate(c, u, u0, p, r, Mi, w, z, Kx, Ky, ch_alphas, &
                ch_betas, max_cheby_iters, max_cheby_iters, dev_id)
        ELSE IF(use_fortran_kernels) THEN
            CALL tea_leaf_kernel_cheby_iterate(x_min, x_max, y_min, y_max, u, &
                u0, p, r, Mi, w, z, Kx, Ky, ch_alphas, ch_betas, max_cheby_iters, &
                rx, ry, 1, tl_preconditioner_on, dev_id)
        ENDIF

        IF(use_ext_kernels) THEN
            CALL ext_calculate_2norm(c, r, error, dev_id)
        ELSE IF(use_fortran_kernels) THEN
            CALL tea_leaf_calc_2norm_kernel(x_min, x_max, y_min, y_max, r, error, dev_id)
        ENDIF

        CALL tea_allsum(error)

        it_alpha = EPSILON(1.0_8)*bb/(4.0_8*error)
        gamm = (SQRT(cn) - 1.0_8)/(SQRT(cn) + 1.0_8)
        est_itc = NINT(LOG(it_alpha)/(2.0_8*LOG(gamm)))

        IF (parallel%boss) THEN
            WRITE(g_out,'(a11)')"est itc"
            WRITE(g_out,'(11i11)')est_itc
            WRITE(0,'(a11)')"est itc"
            WRITE(0,'(11i11)')est_itc
        ENDIF

    END SUBROUTINE tea_leaf_cheby_first_step

END MODULE tea_leaf_module
