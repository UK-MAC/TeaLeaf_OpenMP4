!Crown Copyright 2014 AWE.
!
! This file is part of TeaLeaf.
!
! TeaLeaf is free software: you can redistribute it and/or modify it under 
! the terms of the GNU General Public License as published by the 
! Free Software Foundation, either version 3 of the License, or (at your option) 
! any later version.
!
! TeaLeaf is distributed in the hope that it will be useful, but 
! WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or 
! FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License for more 
! details.
!
! You should have received a copy of the GNU General Public License along with 
! TeaLeaf. If not, see http://www.gnu.org/licenses/.

!>  @brief Fortran heat conduction kernel
!>  @author David Beckingsale, Wayne Gaudin
!>  @details Implicitly calculates the change in temperature using a Jacobi iteration

MODULE tea_leaf_kernel_module

CONTAINS

    SUBROUTINE tea_leaf_kernel_init(x_min,             &
            x_max,             &
            y_min,             &
            y_max,             &
            density,           &
            energy,            &
            u0,                &
            u1,                &
            un,                &
            Kx,                &
            Ky,                &
            coef, &
            dev_id)

        IMPLICIT NONE

        INTEGER         ::            CONDUCTIVITY        = 1 &
            ,RECIP_CONDUCTIVITY  = 2

        INTEGER(KIND=4):: x_min,x_max,y_min,y_max
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: &
            density, energy, u0, Kx, Ky, u1, un

        INTEGER(KIND=4) :: coef

        INTEGER(KIND=4) :: j,k,n,l,dev_id


        ! CALC DIFFUSION COEFFICIENT
        !$OMP TARGET DEVICE(dev_id)
        !$OMP PARALLEL
        IF(coef .EQ. RECIP_CONDUCTIVITY) THEN
            !$OMP DO 
            DO k=y_min-1,y_max+2
                DO j=x_min-1,x_max+2
                    un(j, k)=1.0_8/density(j, k)
                ENDDO
            ENDDO
            !$OMP END DO
        ELSE IF(coef .EQ. CONDUCTIVITY) THEN
            !$OMP DO
            DO k=y_min-1,y_max+2
                DO j=x_min-1,x_max+2
                    un(j, k)=density(j, k)
                ENDDO
            ENDDO
            !$OMP END DO
        ENDIF

        !$OMP DO
        DO k=y_min,y_max+1
            DO j=x_min,x_max+1
                Kx(j, k)=(un(j-1,k  )+un(j, k))/(2.0_8*un(j-1,k  )*un(j, k))
                Ky(j, k)=(un(j  ,k-1)+un(j, k))/(2.0_8*un(j,  k-1)*un(j, k))
            ENDDO
        ENDDO
        !$OMP END DO

        !$OMP DO 
        DO k=y_min-1, y_max+1
            DO j=x_min-1, x_max+1
                u0(j, k) = energy(j, k) * density(j, k)
                u1(j, k) = u0(j, k)
            ENDDO
        ENDDO
        !$OMP END DO
        !$OMP END PARALLEL
        !$OMP END TARGET

    END SUBROUTINE tea_leaf_kernel_init

    SUBROUTINE tea_leaf_kernel_solve(x_min,       &
            x_max,             &
            y_min,             &
            y_max,             &
            rx,                &
            ry,                &
            Kx,                &
            Ky,                &
            error,           &
            u0,                &
            u1,                &
            un, &
            dev_id)


        IMPLICIT NONE

        INTEGER(KIND=4):: x_min,x_max,y_min,y_max
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: u0, un, u1, Kx, Ky

        REAL(KIND=8) :: ry,rx,error,                     solve_time, time, timer

        INTEGER(KIND=4) :: j,k,dev_id


        error = 0.0_8
        !$OMP TARGET DEVICE(dev_id)
        !$OMP PARALLEL
        !$OMP DO
        DO k=y_min-1, y_max+1
            DO j=x_min-1, x_max+1
                un(j, k) = u1(j, k)
            ENDDO
        ENDDO
        !$OMP END DO

        !$OMP DO REDUCTION(+:error)
        DO k=y_min, y_max
            DO j=x_min, x_max
                u1(j, k) = (u0(j, k) + rx*(Kx(j+1,k  )*un(j+1,k  ) + Kx(j, k)*un(j-1,k  )) &
                    + ry*(Ky(j  ,k+1)*un(j  ,k+1) + Ky(j, k)*un(j  ,k-1))) &
                    /(1.0_8 &
                    + rx*(Kx(j, k)+Kx(j+1,k)) &
                    + ry*(Ky(j, k)+Ky(j,k+1)))

                error = error +  ABS(u1(j, k)-un(j, k))
            ENDDO
        ENDDO
        !$OMP END DO
        !$OMP END PARALLEL
        !$OMP END TARGET

    END SUBROUTINE tea_leaf_kernel_solve

    ! Finalise routine is used by both implementations
    SUBROUTINE tea_leaf_kernel_finalise(x_min,    &
            x_max,             &
            y_min,             &
            y_max,             &
            energy,            &
            density,           &
            u, &
            dev_id)

        IMPLICIT NONE

        INTEGER(KIND=4):: x_min,x_max,y_min,y_max
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: u
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: energy
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: density

        INTEGER(KIND=4) :: j,k,dev_id

        !$OMP TARGET DEVICE(dev_id)
        !$OMP PARALLEL DO 
        DO k=y_min, y_max
            DO j=x_min, x_max
                energy(j, k) = u(j, k) / density(j, k)
            ENDDO
        ENDDO
        !$OMP END PARALLEL DO
        !$OMP END TARGET

    END SUBROUTINE tea_leaf_kernel_finalise

    SUBROUTINE tea_leaf_calc_residual(x_min,       &
            x_max,             &
            y_min,             &
            y_max,             &
            u ,                &
            u0,                &
            r,                &
            Kx,                &
            Ky,                &
            rx, ry, &
            dev_id)

        IMPLICIT NONE

        INTEGER(KIND=4):: x_min,x_max,y_min,y_max
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: u0, u, r
        REAL(KIND=8), DIMENSION(x_min-2:x_max+2,y_min-2:y_max+2) :: Kx, ky

        REAL(KIND=8) :: smvp, rx, ry

        INTEGER(KIND=4) :: j,k,dev_id

        !$OMP TARGET DEVICE(dev_id)
        !$OMP PARALLEL DO PRIVATE(smvp)
        DO k=y_min,y_max
            DO j=x_min,x_max
                smvp = (1.0_8                                      &
                    + rx*(Kx(j+1, k) + Kx(j, k)) &
                    + ry*(Ky(j, k+1) + Ky(j, k)))*u(j, k)             &
                    - rx*(Kx(j+1, k)*u(j+1, k) + Kx(j, k)*u(j-1, k)) &
                    - ry*(Ky(j, k+1)*u(j, k+1) + Ky(j, k)*u(j, k-1))
                r(j, k) = u0(j, k) - smvp
            ENDDO
        ENDDO
        !$OMP END PARALLEL DO
        !$OMP END TARGET

    END SUBROUTINE tea_leaf_calc_residual

END MODULE tea_leaf_kernel_module

