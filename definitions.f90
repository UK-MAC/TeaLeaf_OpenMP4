...
MODULE definitions_module
   ...
   !$OMP DECLARE TARGET(step, error_condition, test_problem, complete, use_fortran_kernels)
   !$OMP DECLARE TARGET(use_ext_kernels, tl_use_chebyshev, tl_use_cg, tl_use_ppcg, tl_use_jacobi)
   !$OMP DECLARE TARGET(max_iters, eps, coefficient, tl_ch_cg_errswitch, tl_ch_cg_epslim)
   !$OMP DECLARE TARGET(tl_ch_cg_presteps, tl_check_result, tl_ppcg_inner_steps, tl_preconditioner_on)
   !$OMP DECLARE TARGET(profiler_on, profiler, dt, dx, dy, chunks)
