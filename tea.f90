...
CONTAINS
    ...
    SUBROUTINE tea_allocate_buffers(chunk)
        ...
        IF(parallel%task.EQ.chunks(chunk)%task)THEN
            ...
            ALLOCATE(chunks(chunk)%left_snd_buffer(6*2*(chunks(chunk)%field%y_max+5)))
            ALLOCATE(chunks(chunk)%left_rcv_buffer(6*2*(chunks(chunk)%field%y_max+5)))
            !ENDIF
            !IF(chunks(chunk)%chunk_neighbours(chunk_right).NE.external_face) THEN
            ALLOCATE(chunks(chunk)%right_snd_buffer(6*2*(chunks(chunk)%field%y_max+5)))
            ALLOCATE(chunks(chunk)%right_rcv_buffer(6*2*(chunks(chunk)%field%y_max+5)))
            !ENDIF
            !IF(chunks(chunk)%chunk_neighbours(chunk_bottom).NE.external_face) THEN
            ALLOCATE(chunks(chunk)%bottom_snd_buffer(6*2*(chunks(chunk)%field%x_max+5)))
            ALLOCATE(chunks(chunk)%bottom_rcv_buffer(6*2*(chunks(chunk)%field%x_max+5)))
            !ENDIF
            !IF(chunks(chunk)%chunk_neighbours(chunk_top).NE.external_face) THEN
            ALLOCATE(chunks(chunk)%top_snd_buffer(6*2*(chunks(chunk)%field%x_max+5)))
            ALLOCATE(chunks(chunk)%top_rcv_buffer(6*2*(chunks(chunk)%field%x_max+5)))
            !ENDIF
        ENDIF

    END SUBROUTINE tea_allocate_buffers

    SUBROUTINE tea_exchange(fields,depth,offload)

        IMPLICIT NONE

        INTEGER      :: fields(NUM_FIELDS),depth, chunk, offload
        ...
        IF(chunks(chunk)%chunk_neighbours(chunk_left).NE.external_face) THEN
            ...
            if (use_ext_kernels) then
                call ext_pack_message(chunk,&
                    ...
                    fields, left_right_offset, depth, &
                    CHUNK_LEFT, chunks(chunk)%left_snd_buffer,offload)
            else
                CALL tea_pack_left(chunk, fields, depth, left_right_offset, offload)
            endif
        ...
        IF(chunks(chunk)%chunk_neighbours(chunk_right).NE.external_face) THEN
            ...
            if (use_ext_kernels) then
                call ext_pack_message(chunk,&
                    ...
                    chunks(chunk)%field%vector_sd,&
                    fields, left_right_offset, depth, &
                    CHUNK_RIGHT, chunks(chunk)%right_snd_buffer,offload)
            else
                CALL tea_pack_right(chunk, fields, depth, left_right_offset, offload)
            endif
        ...
        IF(chunks(chunk)%chunk_neighbours(chunk_left).NE.external_face) THEN
            if (use_ext_kernels) then
                call ext_unpack_message(chunk,&
                    ...
                    chunks(chunk)%field%vector_sd,&
                    fields, left_right_offset, depth, &
                    CHUNK_LEFT, chunks(chunk)%left_rcv_buffer,offload)
            else
        ...
        IF(chunks(chunk)%chunk_neighbours(chunk_right).NE.external_face) THEN
            if (use_ext_kernels) then
                call ext_unpack_message(chunk,&
                    ...
                    chunks(chunk)%field%vector_sd,&
                    fields, left_right_offset, depth, &
                    CHUNK_RIGHT, chunks(chunk)%right_rcv_buffer,offload)
            else
        ...
        IF(chunks(chunk)%chunk_neighbours(chunk_bottom).NE.external_face) THEN
            ...
            if (use_ext_kernels) then
                call ext_pack_message(chunk,&
                    ...
                    chunks(chunk)%field%vector_p,&
                    chunks(chunk)%field%vector_sd,&
                    fields, bottom_top_offset, depth, &
                    CHUNK_BOTTOM, chunks(chunk)%bottom_snd_buffer,offload)
            else
                CALL tea_pack_bottom(chunk, fields, depth, bottom_top_offset, offload)
            endif
        ...
        IF(chunks(chunk)%chunk_neighbours(chunk_top).NE.external_face) THEN
            ...
            if (use_ext_kernels) then
                call ext_pack_message(chunk,&
                    ...
                    chunks(chunk)%field%vector_sd,&
                    fields, bottom_top_offset, depth, &
                    CHUNK_TOP, chunks(chunk)%top_snd_buffer,offload)
            else
                CALL tea_pack_top(chunk, fields, depth, bottom_top_offset, offload)
            endif
        ...
        IF( chunks(chunk)%chunk_neighbours(chunk_top).NE.external_face ) THEN
            if (use_ext_kernels) then
                call ext_unpack_message(chunk,&
                    ...
                    chunks(chunk)%field%vector_sd,&
                    fields, bottom_top_offset, depth, &
                    CHUNK_TOP, chunks(chunk)%top_rcv_buffer,offload)
            else
        ...
        IF(chunks(chunk)%chunk_neighbours(chunk_bottom).NE.external_face) THEN
            if (use_ext_kernels) then
                call ext_unpack_message(chunk,&
                    ...
                    chunks(chunk)%field%vector_sd,&
                    fields, bottom_top_offset, depth, &
                    CHUNK_BOTTOM, chunks(chunk)%bottom_rcv_buffer,offload)
            else
    ...
    SUBROUTINE tea_pack_left(chunk, fields, depth, left_right_offset, offload)
        ...
        IF(fields(FIELD_DENSITY).EQ.1) THEN
            IF(use_fortran_kernels) THEN
                CALL tea_pack_message_left(chunks(chunk)%field%x_min,                    &
                    ...
                    depth, CELL_DATA,                             &
                    left_right_offset(FIELD_DENSITY),offload,parallel%dev_id)
            ENDIF
        ENDIF
        IF(fields(FIELD_ENERGY0).EQ.1) THEN
            IF(use_fortran_kernels) THEN
                CALL tea_pack_message_left(chunks(chunk)%field%x_min,                    &
                    ...
                    CELL_DATA,VERTEX_DATA,X_FACE_DATA,Y_FACE_DATA,&
                    depth, CELL_DATA,                             &
                    left_right_offset(FIELD_ENERGY0),offload,parallel%dev_id)
            ENDIF
        ENDIF
        IF(fields(FIELD_ENERGY1).EQ.1) THEN
            IF(use_fortran_kernels) THEN
                CALL tea_pack_message_left(chunks(chunk)%field%x_min,                    &
                    ...
                    CELL_DATA,VERTEX_DATA,X_FACE_DATA,Y_FACE_DATA,&
                    depth, CELL_DATA,                             &
                    left_right_offset(FIELD_ENERGY1),offload,parallel%dev_id)
            ENDIF
        ENDIF
        IF(fields(FIELD_P).EQ.1) THEN
            IF(use_fortran_kernels) THEN
                CALL tea_pack_message_left(chunks(chunk)%field%x_min,                    &
                    ...
                    CELL_DATA,VERTEX_DATA,X_FACE_DATA,Y_FACE_DATA,&
                    depth, CELL_DATA,                             &
                    left_right_offset(FIELD_P),offload,parallel%dev_id)
            ENDIF
        ENDIF
        IF(fields(FIELD_U).EQ.1) THEN
            IF(use_fortran_kernels) THEN
                CALL tea_pack_message_left(chunks(chunk)%field%x_min,                    &
                    ...
                    CELL_DATA,VERTEX_DATA,X_FACE_DATA,Y_FACE_DATA,&
                    depth, CELL_DATA,                             &
                    left_right_offset(FIELD_U),offload,parallel%dev_id)
            ENDIF
        ENDIF
        IF(fields(FIELD_SD).EQ.1) THEN
            IF(use_fortran_kernels) THEN
                CALL tea_pack_message_left(chunks(chunk)%field%x_min,                    &
                    ...
                    CELL_DATA,VERTEX_DATA,X_FACE_DATA,Y_FACE_DATA,&
                    depth, CELL_DATA,                             &
                    left_right_offset(FIELD_SD),offload,parallel%dev_id)
            ENDIF
        ENDIF
    ...
    SUBROUTINE tea_unpack_left(fields, chunk, depth,                         &
        ...
        INTEGER         :: fields(:), chunk, depth, offload
        INTEGER         :: left_right_offset(:)
    ...
    SUBROUTINE tea_pack_right(chunk, fields, depth, left_right_offset, offload)
    ...
    SUBROUTINE tea_pack_top(chunk, fields, depth, bottom_top_offset,offload)
        ...
        IF(fields(FIELD_DENSITY).EQ.1) THEN
            IF(use_fortran_kernels) THEN
                CALL tea_pack_message_top(chunks(chunk)%field%x_min,                    &
                    ...
                    CELL_DATA,VERTEX_DATA,X_FACE_DATA,Y_FACE_DATA,&
                    depth, CELL_DATA,                             &
                    bottom_top_offset(FIELD_DENSITY),offload,parallel%dev_id)
            ENDIF
        ENDIF
        IF(fields(FIELD_ENERGY0).EQ.1) THEN
            IF(use_fortran_kernels) THEN
                CALL tea_pack_message_top(chunks(chunk)%field%x_min,                    &
                    ...
                    CELL_DATA,VERTEX_DATA,X_FACE_DATA,Y_FACE_DATA,&
                    depth, CELL_DATA,                             &
                    bottom_top_offset(FIELD_ENERGY0),offload,parallel%dev_id)
            ENDIF
        ENDIF
        IF(fields(FIELD_ENERGY1).EQ.1) THEN
            IF(use_fortran_kernels) THEN
                CALL tea_pack_message_top(chunks(chunk)%field%x_min,                    &
                    ...
                    CELL_DATA,VERTEX_DATA,X_FACE_DATA,Y_FACE_DATA,&
                    depth, CELL_DATA,                             &
                    bottom_top_offset(FIELD_ENERGY1),offload,parallel%dev_id)
            ENDIF
        ENDIF
        IF(fields(FIELD_P).EQ.1) THEN
            IF(use_fortran_kernels) THEN
                CALL tea_pack_message_top(chunks(chunk)%field%x_min,                    &
                    ...
                    CELL_DATA,VERTEX_DATA,X_FACE_DATA,Y_FACE_DATA,&
                    depth, CELL_DATA,                             &
                    bottom_top_offset(FIELD_P),offload,parallel%dev_id)
            ENDIF
        ENDIF
        IF(fields(FIELD_U).EQ.1) THEN
            IF(use_fortran_kernels) THEN
                CALL tea_pack_message_top(chunks(chunk)%field%x_min,                    &
                    ...
                    CELL_DATA,VERTEX_DATA,X_FACE_DATA,Y_FACE_DATA,&
                    depth, CELL_DATA,                             &
                    bottom_top_offset(FIELD_U),offload,parallel%dev_id)
            ENDIF
        ENDIF
        IF(fields(FIELD_SD).EQ.1) THEN
            IF(use_fortran_kernels) THEN
                CALL tea_pack_message_top(chunks(chunk)%field%x_min,                    &
                    ...
                    CELL_DATA,VERTEX_DATA,X_FACE_DATA,Y_FACE_DATA,&
                    depth, CELL_DATA,                             &
                    bottom_top_offset(FIELD_SD),offload,parallel%dev_id)
            ENDIF
        ENDIF
    ...
    SUBROUTINE tea_unpack_top(fields, chunk, depth, top_rcv_buffer, &
            bottom_top_offset, offload)
    ...
    SUBROUTINE tea_pack_bottom(chunk, fields, depth, bottom_top_offset,offload)
    ...
    SUBROUTINE tea_unpack_bottom(fields, chunk, depth, bottom_rcv_buffer, &
            bottom_top_offset,offload)
