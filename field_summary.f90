...
SUBROUTINE field_summary()
    ...
    ELSEIF(use_ext_kernels) THEN
        DO c=1,chunks_per_task
        IF(chunks(c)%task.EQ.parallel%task) THEN
            CALL ext_field_summary_kernel(c, &
                chunks(c)%field%volume,                  &
                chunks(c)%field%density,                 &
                chunks(c)%field%energy0,                 &
                chunks(c)%field%u,                       &
    ...
    IF(complete) THEN
        IF(parallel%boss) THEN
            ...
            IF(test_problem.GE.1) THEN
                ...
                IF(test_problem.EQ.6) qa_diff=ABS((100.0_8*(temp/103.88639125996923_8))-100.0_8)

                ! A number of additional test problems, run without IEEE compliance
